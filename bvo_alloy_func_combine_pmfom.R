alloyplots <- function(pmfom, alloys = c("C", "D", "E", "F", "G", "H"), fom = "pmax",
    titlestring = "labelme", aggmethod = "", yscale = 1, maxy = 1, miny = 0, plot = T,
    sigout = 2) {
    mymed = {
    ...
    # single alloys
    sal <- pmfom[A + B > 0][D + E + F + G + H == 0 | C + E + F + G + H == 0 | C +
        D + F + G + H == 0 | C + D + E + G + H == 0 | C + D + E + F + H == 0 | C +
        D + E + F + G == 0]
    ...
    psal[, `:=`(alloy, "")]
    # group into binaries
    cgrp <- pmfom[A + B > 0][(C + D > 0 & E + F + G + H == 0) | (C + E > 0 & D +
        F + G + H == 0) | (C + F > 0 & D + E + G + H == 0) | (C + G > 0 & D + E +
        F + H == 0) | (C + H > 0 & D + E + F + G == 0)]
    dgrp <- pmfom[A + B > 0][(D + C > 0 & E + F + G + H == 0) | (D + E > 0 & C +
        F + G + H == 0) | (D + F > 0 & C + E + G + H == 0) | (D + G > 0 & C + E +
        F + H == 0) | (D + H > 0 & C + E + F + G == 0)]
    egrp <- pmfom[A + B > 0][(E + C > 0 & D + F + G + H == 0) | (E + D > 0 & C +
        F + G + H == 0) | (E + F > 0 & C + D + G + H == 0) | (E + G > 0 & C + D +
        F + H == 0) | (E + H > 0 & C + D + F + G == 0)]
    fgrp <- pmfom[A + B > 0][(F + C > 0 & D + E + G + H == 0) | (F + D > 0 & C +
        E + G + H == 0) | (F + E > 0 & C + D + G + H == 0) | (F + G > 0 & C + D +
        E + H == 0) | (F + H > 0 & C + D + E + G == 0)]
    ggrp <- pmfom[A + B > 0][(G + C > 0 & D + E + F + H == 0) | (G + D > 0 & C +
        E + F + H == 0) | (G + E > 0 & C + D + F + H == 0) | (G + F > 0 & C + D +
        E + H == 0) | (G + H > 0 & C + D + E + F == 0)]
    hgrp <- pmfom[A + B > 0][(H + C > 0 & D + E + F + G == 0) | (H + D > 0 & C +
        E + F + G == 0) | (H + E > 0 & C + D + F + G == 0) | (H + F > 0 & C + D +
        E + G == 0) | (H + G > 0 & C + D + E + F == 0)]
    ...
    if (plot) {
        splot <- ggplot(psal, aes(x = Yval, y = pmed * yscale, color = variable)) +
            theme_bw() + geom_line(aes(group = variable)) + geom_point() + facet_grid(alloy ~
            vx_lab, switch = "y") + xlab(expression(y ~ "in" ~ Bi[1 - x] * V[x] *
            O[4 + delta]:A[y])) + ylab(y_label) + scale_y_continuous(breaks = seq(miny,
            maxy, length.out = 5), limits = c(miny, maxy)) + scale_color_manual(name = expression(A),
        ...
        bplot <- ggplot(bsub, aes(x = alloy_frac, y = pmed * yscale, color = variable,
            ...
            scale_y_continuous(breaks = seq(miny, maxy, length.out = 5), limits = c(miny,
                maxy)) + scale_shape_manual(name = expression(y), values = 1:4) +
            scale_color_manual(name = expression(B), values = gg.colors(6)) + theme(legend.position = "right",
            strip.background = element_blank(), strip.text.x = element_blank(), axis.text.y = element_blank(),
            axis.title.y = element_blank(), text = element_text(size = 16), panel.spacing = unit(0.8,
                "lines"))
