alloyplots<-function(pmfom, alloys=c('C', 'D', 'E', 'F', 'G', 'H'), fom='pmax', titlestring='labelme', aggmethod='', yscale=1, maxy=1, miny=0, plot=T, sigout=2) {
    mymed={function(x) if(length(x)>1) na.omit(sort(x, decreasing=T))[floor(length(x)/2)] else x}
    if(aggmethod=='top2') {
    ...
    # single alloys
    sal<-pmfom[A+B>0][D+E+F+G+H==0 | C+E+F+G+H==0 | C+D+F+G+H==0 | C+D+E+G+H==0 | C+D+E+F+H==0 | C+D+E+F+G==0]
    salm<-melt(sal[!is.na(eval(usefom))][,.(pmed=aggfunc(eval(usefom)), jmed=aggfunc(jsc), Sample=Sample[which(eval(usefom)==aggfunc(eval(usefom)))], mean_pmed=mean(eval(usefom)), num_smps=.N, stdev_pmed=sd(eval(usefom)), mean_pmed_nsig=mean(eval(usefom)[eval(usefom)>=mean(eval(usefom))-sigout*sd(eval(usefom)) & eval(usefom)<=mean(eval(usefom))+sigout*sd(eval(usefom))]), stdev_pmed_nsig=sd(eval(usefom)[eval(usefom)>=mean(eval(usefom))-sigout*sd(eval(usefom)) & eval(usefom)<=mean(eval(usefom))+sigout*sd(eval(usefom))])), by=list(Bi, V, C_, D_, E_, F_, G_, H_, alloy_sum, BiV_ratio)], id.vars=c('Bi', 'V', 'pmed', 'jmed', 'alloy_sum', 'BiV_ratio', 'mean_pmed', 'stdev_pmed', 'Sample', 'mean_pmed_nsig', 'stdev_pmed_nsig', 'num_smps'))
    salm[,variable:=factor(variable, levels=c('C_', 'D_', 'E_', 'F_', 'G_', 'H_'), labels=alloys)]

    # pures
    pur<-pmfom[A+B>0][C+D+E+F+G+H==0]
    purm<-melt(pur[!is.na(eval(usefom))][,.(pmed=aggfunc(eval(usefom)), jmed=aggfunc(jsc), Sample=Sample[which(eval(usefom)==aggfunc(eval(usefom)))], mean_pmed=mean(eval(usefom)), num_smps=.N, stdev_pmed=sd(eval(usefom)), mean_pmed_nsig=mean(eval(usefom)[eval(usefom)>=mean(eval(usefom))-sigout*sd(eval(usefom)) & eval(usefom)<=mean(eval(usefom))+sigout*sd(eval(usefom))]), stdev_pmed_nsig=sd(eval(usefom)[eval(usefom)>=mean(eval(usefom))-sigout*sd(eval(usefom)) & eval(usefom)<=mean(eval(usefom))+sigout*sd(eval(usefom))])), by=list(Bi, V, C_, D_, E_, F_, G_, H_, alloy_sum, BiV_ratio)], id.vars=c('Bi', 'V', 'pmed', 'jmed', 'alloy_sum', 'BiV_ratio', 'mean_pmed', 'stdev_pmed', 'Sample', 'mean_pmed_nsig', 'stdev_pmed_nsig', 'num_smps'))
    ...
    cgrp<-pmfom[A+B>0][(C+D>0 & E+F+G+H==0) |
                                       (C+E>0 & D+F+G+H==0) |
                                       (C+F>0 & D+E+G+H==0) |
                                       (C+G>0 & D+E+F+H==0) |
                                       (C+H>0 & D+E+F+G==0)]

    dgrp<-pmfom[A+B>0][(D+C>0 & E+F+G+H==0) |
                                       (D+E>0 & C+F+G+H==0) |
                                       (D+F>0 & C+E+G+H==0) |
                                       (D+G>0 & C+E+F+H==0) |
                                       (D+H>0 & C+E+F+G==0)]

    egrp<-pmfom[A+B>0][(E+C>0 & D+F+G+H==0) |
                                       (E+D>0 & C+F+G+H==0) |
                                       (E+F>0 & C+D+G+H==0) |
                                       (E+G>0 & C+D+F+H==0) |
                                       (E+H>0 & C+D+F+G==0)]

    fgrp<-pmfom[A+B>0][(F+C>0 & D+E+G+H==0) |
                                       (F+D>0 & C+E+G+H==0) |
                                       (F+E>0 & C+D+G+H==0) |
                                       (F+G>0 & C+D+E+H==0) |
                                       (F+H>0 & C+D+E+G==0)]

    ggrp<-pmfom[A+B>0][(G+C>0 & D+E+F+H==0) |
                                       (G+D>0 & C+E+F+H==0) |
                                       (G+E>0 & C+D+F+H==0) |
                                       (G+F>0 & C+D+E+H==0) |
                                       (G+H>0 & C+D+E+F==0)]

    hgrp<-pmfom[A+B>0][(H+C>0 & D+E+F+G==0) |
                                       (H+D>0 & C+E+F+G==0) |
                                       (H+E>0 & C+D+F+G==0) |
                                       (H+F>0 & C+D+E+G==0) |
                                       (H+G>0 & C+D+E+F==0)]
    ...
    cmelt<-melt(cgrp[!is.na(eval(usefom))][,.(pmed=aggfunc(eval(usefom)), jmed=aggfunc(jsc), Sample=Sample[which(eval(usefom)==aggfunc(eval(usefom)))], mean_pmed=mean(eval(usefom)), num_smps=.N, stdev_pmed=sd(eval(usefom)), mean_pmed_nsig=mean(eval(usefom)[eval(usefom)>=mean(eval(usefom))-sigout*sd(eval(usefom)) & eval(usefom)<=mean(eval(usefom))+sigout*sd(eval(usefom))]), stdev_pmed_nsig=sd(eval(usefom)[eval(usefom)>=mean(eval(usefom))-sigout*sd(eval(usefom)) & eval(usefom)<=mean(eval(usefom))+sigout*sd(eval(usefom))])), by=list(Bi, V, C_, D_, E_, F_, G_, H_, alloy_sum, alloy_frac, BiV_ratio)], id.vars=c('Bi', 'V', 'pmed', 'jmed', 'alloy_sum', 'alloy_frac', 'BiV_ratio', 'mean_pmed', 'stdev_pmed', 'Sample', 'mean_pmed_nsig', 'stdev_pmed_nsig', 'num_smps'))
    dmelt<-melt(dgrp[!is.na(eval(usefom))][,.(pmed=aggfunc(eval(usefom)), jmed=aggfunc(jsc), Sample=Sample[which(eval(usefom)==aggfunc(eval(usefom)))], mean_pmed=mean(eval(usefom)), num_smps=.N, stdev_pmed=sd(eval(usefom)), mean_pmed_nsig=mean(eval(usefom)[eval(usefom)>=mean(eval(usefom))-sigout*sd(eval(usefom)) & eval(usefom)<=mean(eval(usefom))+sigout*sd(eval(usefom))]), stdev_pmed_nsig=sd(eval(usefom)[eval(usefom)>=mean(eval(usefom))-sigout*sd(eval(usefom)) & eval(usefom)<=mean(eval(usefom))+sigout*sd(eval(usefom))])), by=list(Bi, V, C_, D_, E_, F_, G_, H_, alloy_sum, alloy_frac, BiV_ratio)], id.vars=c('Bi', 'V', 'pmed', 'jmed', 'alloy_sum', 'alloy_frac', 'BiV_ratio', 'mean_pmed', 'stdev_pmed', 'Sample', 'mean_pmed_nsig', 'stdev_pmed_nsig', 'num_smps'))
    emelt<-melt(egrp[!is.na(eval(usefom))][,.(pmed=aggfunc(eval(usefom)), jmed=aggfunc(jsc), Sample=Sample[which(eval(usefom)==aggfunc(eval(usefom)))], mean_pmed=mean(eval(usefom)), num_smps=.N, stdev_pmed=sd(eval(usefom)), mean_pmed_nsig=mean(eval(usefom)[eval(usefom)>=mean(eval(usefom))-sigout*sd(eval(usefom)) & eval(usefom)<=mean(eval(usefom))+sigout*sd(eval(usefom))]), stdev_pmed_nsig=sd(eval(usefom)[eval(usefom)>=mean(eval(usefom))-sigout*sd(eval(usefom)) & eval(usefom)<=mean(eval(usefom))+sigout*sd(eval(usefom))])), by=list(Bi, V, C_, D_, E_, F_, G_, H_, alloy_sum, alloy_frac, BiV_ratio)], id.vars=c('Bi', 'V', 'pmed', 'jmed', 'alloy_sum', 'alloy_frac', 'BiV_ratio', 'mean_pmed', 'stdev_pmed', 'Sample', 'mean_pmed_nsig', 'stdev_pmed_nsig', 'num_smps'))
    fmelt<-melt(fgrp[!is.na(eval(usefom))][,.(pmed=aggfunc(eval(usefom)), jmed=aggfunc(jsc), Sample=Sample[which(eval(usefom)==aggfunc(eval(usefom)))], mean_pmed=mean(eval(usefom)), num_smps=.N, stdev_pmed=sd(eval(usefom)), mean_pmed_nsig=mean(eval(usefom)[eval(usefom)>=mean(eval(usefom))-sigout*sd(eval(usefom)) & eval(usefom)<=mean(eval(usefom))+sigout*sd(eval(usefom))]), stdev_pmed_nsig=sd(eval(usefom)[eval(usefom)>=mean(eval(usefom))-sigout*sd(eval(usefom)) & eval(usefom)<=mean(eval(usefom))+sigout*sd(eval(usefom))])), by=list(Bi, V, C_, D_, E_, F_, G_, H_, alloy_sum, alloy_frac, BiV_ratio)], id.vars=c('Bi', 'V', 'pmed', 'jmed', 'alloy_sum', 'alloy_frac', 'BiV_ratio', 'mean_pmed', 'stdev_pmed', 'Sample', 'mean_pmed_nsig', 'stdev_pmed_nsig', 'num_smps'))
    gmelt<-melt(ggrp[!is.na(eval(usefom))][,.(pmed=aggfunc(eval(usefom)), jmed=aggfunc(jsc), Sample=Sample[which(eval(usefom)==aggfunc(eval(usefom)))], mean_pmed=mean(eval(usefom)), num_smps=.N, stdev_pmed=sd(eval(usefom)), mean_pmed_nsig=mean(eval(usefom)[eval(usefom)>=mean(eval(usefom))-sigout*sd(eval(usefom)) & eval(usefom)<=mean(eval(usefom))+sigout*sd(eval(usefom))]), stdev_pmed_nsig=sd(eval(usefom)[eval(usefom)>=mean(eval(usefom))-sigout*sd(eval(usefom)) & eval(usefom)<=mean(eval(usefom))+sigout*sd(eval(usefom))])), by=list(Bi, V, C_, D_, E_, F_, G_, H_, alloy_sum, alloy_frac, BiV_ratio)], id.vars=c('Bi', 'V', 'pmed', 'jmed', 'alloy_sum', 'alloy_frac', 'BiV_ratio', 'mean_pmed', 'stdev_pmed', 'Sample', 'mean_pmed_nsig', 'stdev_pmed_nsig', 'num_smps'))
    hmelt<-melt(hgrp[!is.na(eval(usefom))][,.(pmed=aggfunc(eval(usefom)), jmed=aggfunc(jsc), Sample=Sample[which(eval(usefom)==aggfunc(eval(usefom)))], mean_pmed=mean(eval(usefom)), num_smps=.N, stdev_pmed=sd(eval(usefom)), mean_pmed_nsig=mean(eval(usefom)[eval(usefom)>=mean(eval(usefom))-sigout*sd(eval(usefom)) & eval(usefom)<=mean(eval(usefom))+sigout*sd(eval(usefom))]), stdev_pmed_nsig=sd(eval(usefom)[eval(usefom)>=mean(eval(usefom))-sigout*sd(eval(usefom)) & eval(usefom)<=mean(eval(usefom))+sigout*sd(eval(usefom))])), by=list(Bi, V, C_, D_, E_, F_, G_, H_, alloy_sum, alloy_frac, BiV_ratio)], id.vars=c('Bi', 'V', 'pmed', 'jmed', 'alloy_sum', 'alloy_frac', 'BiV_ratio', 'mean_pmed', 'stdev_pmed', 'Sample', 'mean_pmed_nsig', 'stdev_pmed_nsig', 'num_smps'))
    ...
    if(plot) {
        ...
        bplot<-ggplot(bsub, aes(x=alloy_frac, y=pmed*yscale, color=variable, shape=factor(Yval))) +
            theme_bw() +    
            geom_point() +
            geom_line(aes(group=interaction(variable, factor(Yval)))) +
