alloyplots<-function(fomdt, alloys=c('C', 'D', 'E', 'F', 'G', 'H'), fom='pmax', titlestring='labelme', aggmethod='', yscale=1E3/0.0058, maxy=1, miny=0, plot=T) {
    
    if(aggmethod=='top2') {
        aggfunc={function(x) mean(na.omit(sort(x,decreasing=T)[1:2]))}
    }
    else if (aggmethod=='mymed'){
        aggfunc={function(x) if(length(x)>1) na.omit(sort(x, decreasing=T))[floor(length(x)/2)] else x}
    }
    else {
        aggfunc={function(x) median(na.omit(x))}
    }
    ...
    # single alloys
    sal<-pm[fomdt][A+B>0][D+E+F+G+H==0 | C+E+F+G+H==0 | C+D+F+G+H==0 | C+D+E+G+H==0 | C+D+E+F+H==0 | C+D+E+F+G==0]
    salm<-melt(sal[!is.na(eval(usefom))][,.(pmed=aggfunc(eval(usefom)), jmed=aggfunc(jsc)), by=list(Bi, V, C_, D_, E_, F_, G_, H_, alloy_sum, BiV_ratio)], id.vars=c('Bi', 'V', 'pmed', 'jmed', 'alloy_sum', 'BiV_ratio'))
    salm[,variable:=factor(variable, levels=c('C_', 'D_', 'E_', 'F_', 'G_', 'H_'), labels=alloys)]

    # pures
    pur<-pm[fomdt][A+B>0][C+D+E+F+G+H==0]
    purm<-melt(pur[!is.na(eval(usefom))][,.(pmed=aggfunc(eval(usefom)), jmed=aggfunc(jsc)), by=list(Bi, V, C_, D_, E_, F_, G_, H_, alloy_sum, BiV_ratio)], id.vars=c('Bi', 'V', 'pmed', 'jmed', 'alloy_sum', 'BiV_ratio'))
    ...
    cgrp<-pm[fomdt][A+B>0][(C+D>0 & E+F+G+H==0) |
                                       (C+E>0 & D+F+G+H==0) |
                                       (C+F>0 & D+E+G+H==0) |
                                       (C+G>0 & D+E+F+H==0) |
                                       (C+H>0 & D+E+F+G==0)]

    dgrp<-pm[fomdt][A+B>0][(D+C>0 & E+F+G+H==0) |
                                       (D+E>0 & C+F+G+H==0) |
                                       (D+F>0 & C+E+G+H==0) |
                                       (D+G>0 & C+E+F+H==0) |
                                       (D+H>0 & C+E+F+G==0)]

    egrp<-pm[fomdt][A+B>0][(E+C>0 & D+F+G+H==0) |
                                       (E+D>0 & C+F+G+H==0) |
                                       (E+F>0 & C+D+G+H==0) |
                                       (E+G>0 & C+D+F+H==0) |
                                       (E+H>0 & C+D+F+G==0)]

    fgrp<-pm[fomdt][A+B>0][(F+C>0 & D+E+G+H==0) |
                                       (F+D>0 & C+E+G+H==0) |
                                       (F+E>0 & C+D+G+H==0) |
                                       (F+G>0 & C+D+E+H==0) |
                                       (F+H>0 & C+D+E+G==0)]

    ggrp<-pm[fomdt][A+B>0][(G+C>0 & D+E+F+H==0) |
                                       (G+D>0 & C+E+F+H==0) |
                                       (G+E>0 & C+D+F+H==0) |
                                       (G+F>0 & C+D+E+H==0) |
                                       (G+H>0 & C+D+E+F==0)]

    hgrp<-pm[fomdt][A+B>0][(H+C>0 & D+E+F+G==0) |
                                       (H+D>0 & C+E+F+G==0) |
                                       (H+E>0 & C+D+F+G==0) |
                                       (H+F>0 & C+D+E+G==0) |
                                       (H+G>0 & C+D+E+F==0)]
    ...
    cmelt<-melt(cgrp[!is.na(eval(usefom))][,.(pmed=aggfunc(eval(usefom)), jmed=aggfunc(jsc)), by=list(Bi, V, C_, D_, E_, F_, G_, H_, alloy_sum, alloy_frac, BiV_ratio)], id.vars=c('Bi', 'V', 'pmed', 'jmed', 'alloy_sum', 'alloy_frac', 'BiV_ratio'))
    dmelt<-melt(dgrp[!is.na(eval(usefom))][,.(pmed=aggfunc(eval(usefom)), jmed=aggfunc(jsc)), by=list(Bi, V, C_, D_, E_, F_, G_, H_, alloy_sum, alloy_frac, BiV_ratio)], id.vars=c('Bi', 'V', 'pmed', 'jmed', 'alloy_sum', 'alloy_frac', 'BiV_ratio'))
    emelt<-melt(egrp[!is.na(eval(usefom))][,.(pmed=aggfunc(eval(usefom)), jmed=aggfunc(jsc)), by=list(Bi, V, C_, D_, E_, F_, G_, H_, alloy_sum, alloy_frac, BiV_ratio)], id.vars=c('Bi', 'V', 'pmed', 'jmed', 'alloy_sum', 'alloy_frac', 'BiV_ratio'))
    fmelt<-melt(fgrp[!is.na(eval(usefom))][,.(pmed=aggfunc(eval(usefom)), jmed=aggfunc(jsc)), by=list(Bi, V, C_, D_, E_, F_, G_, H_, alloy_sum, alloy_frac, BiV_ratio)], id.vars=c('Bi', 'V', 'pmed', 'jmed', 'alloy_sum', 'alloy_frac', 'BiV_ratio'))
    gmelt<-melt(ggrp[!is.na(eval(usefom))][,.(pmed=aggfunc(eval(usefom)), jmed=aggfunc(jsc)), by=list(Bi, V, C_, D_, E_, F_, G_, H_, alloy_sum, alloy_frac, BiV_ratio)], id.vars=c('Bi', 'V', 'pmed', 'jmed', 'alloy_sum', 'alloy_frac', 'BiV_ratio'))
    hmelt<-melt(hgrp[!is.na(eval(usefom))][,.(pmed=aggfunc(eval(usefom)), jmed=aggfunc(jsc)), by=list(Bi, V, C_, D_, E_, F_, G_, H_, alloy_sum, alloy_frac, BiV_ratio)], id.vars=c('Bi', 'V', 'pmed', 'jmed', 'alloy_sum', 'alloy_frac', 'BiV_ratio'))
    ...
    biv_labels = c('x = 0.48', 'x = 0.5 (0.498 and 0.500)', 'x = 0.52 (0.519 and 0.524)')
    ...
    splot<-ggplot(psal, aes(x=Yval, y=pmed*yscale, color=variable)) +
        theme_bw() +
        ...
        ylab(y_label) +
        scale_y_continuous(breaks=seq(miny, maxy, length.out=5), limits=c(miny, maxy)) +
    ...
    bplot<-ggplot(bsub, aes(x=alloy_frac, y=pmed*yscale, color=variable, shape=factor(Yval))) +
        theme_bw() +    
        geom_point() +
